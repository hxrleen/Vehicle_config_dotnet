...
const ConfiguratorContainer = (props) => {
  ...
  const fetchSegments = () => {
    fetch("https://localhost:7139/api/Segment")
  ...
  const fetchManufacturers = (segmentId) => {
    fetch(`https://localhost:7139/api/Manufacturer/segment/${segmentId}`)
  ...
  const fetchModels = (segmentId, manufacturerId) => {
    fetch(`https://localhost:7139/api/Model/api/Model/${segmentId}/${manufacturerId}`)
      .then((response) => response.json())
      .then((data) => {
        console.log(data);
        setModels(data);
